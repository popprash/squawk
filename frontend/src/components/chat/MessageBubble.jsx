      ...
      >
        {hasImage ? (
          <img
            ...
            alt=""
            className="mb-1.5 max-h-40 max-w-full rounded-lg object-cover sm:max-h-52 sm:rounded-xl cursor-zoom-in hover:brightness-95 transition-[filter]"
            onClick={() => setLightboxMedia({ type: "image", url: message.imageUrl })}
          />
        ) : null}
        {hasVideo ? (
          <MessageVideo
            src={message.videoUrl}
            onExpand={() => setLightboxMedia({ type: "video", url: message.videoUrl })}
          />
        ) : null}
        {message.text ? (
