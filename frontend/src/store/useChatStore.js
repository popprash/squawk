    ...
    (set, get) => ({
      ...
      subscribeToMessages: () => {
        ...
        socket.on("newMessage", (newMessage) => {
          ...
          // Check if message belongs to the current active conversation
          const isMessageFromActiveChat = activeConversationId && (
          ...
          if (isMessageFromActiveChat) {
            ...
            const isDuplicate = get().messages.some(
              (msg) => String(msg._id || msg.id) === String(newMessage._id || newMessage.id)
            );
            if (!isDuplicate) {
          ...
          // Always fetch conversations to update sidebar list, last message preview, and order in real-time
          get().getConversations();
        });
      },
